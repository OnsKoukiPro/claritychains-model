# src/analytics/forecasting_engine.py
...
class ForecastingEngine:
    ...
    def render_enhanced_forecasting(self, prices_df, use_enhanced_forecasting=True):
            ...
            with st.spinner("Running enhanced forecasting analysis..."):
                try:
                    ...
                    try:
                        ...
                        forecaster = FallbackBaselineForecaster(self.config)
                    ...
                    # Display results
                    self._display_forecast_results(result, material_data, material, show_comparison, use_fundamentals, forecaster)
    ...
    def _display_forecast_results(self, result, material_data, material, show_comparison, use_fundamentals, forecaster):
        """Display forecast results and analysis"""
        ...
        if not forecast_df.empty:
            self._create_forecast_plot(material_data, forecast_df, material)
            self._display_forecast_table(forecast_df)

            # Method comparison if requested
            if show_comparison and use_fundamentals:
                self._display_method_comparison(forecaster, material_data, material)
        else:
            st.warning("No forecast data available")

    def _create_forecast_plot(self, material_data, forecast_df, material):
    ...
    def _display_forecast_table(self, forecast_df):
        """Display forecast details table"""
    ...
    def _display_method_comparison(self, forecaster, material_data, material):
        """Display forecasting method comparison"""
